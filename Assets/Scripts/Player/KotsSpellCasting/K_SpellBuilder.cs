...
public class K_SpellBuilder : NetworkBehaviour
{
    ...
    [Serializable] public class DrDict : UDictionary<KeyCode, K_DRData> { }
    ...
    private Dictionary<KeyCode, int> drTiers;

    // Holds the current DR cooldown for each spell family
    private Dictionary<KeyCode, float> drCooldowns;

    // The last spell type that triggered a DR
    private KeyCode lastDrSpellType;
    ...
    private void Awake()
    {
        ...
        K_SpellData spellData = spellDictionary["Q"];
        baseSpell = spellData.prefab;

        drTiers = new Dictionary<KeyCode, int>();
        drCooldowns = new Dictionary<KeyCode, float>();
        foreach (KeyCode spellType in drDictionary.Keys)
        {
        ...
        // by whether or the DR tier is active and/or higher than 0
    ...
    private void Update()
    {
        ...
        foreach (KeyCode spellType in drCooldowns.Keys.ToList())
        {
            if (drCooldowns[spellType] > 0f)
            {
                ...
                if (drCooldowns[spellType] <= 0)
                {
                    ...
                    isIgnoreDRLock[spellType] = false;
                }
            }
        }
    }
    ...
    public Queue<K_DRKeyData> GetDrKeys(KeyCode spellType)
    {
        ...
        int drTier = drTiers[spellType] - minCasts;

        if (!drDictionary.ContainsKey(spellType) || drTier < 0)
            return new Queue<K_DRKeyData>();

        KeyCode[] keys = FisherYatesShuffle(K_SpellKeys.spellTypes);

        drTier = Math.Clamp(drTier, 0, drDictionary[spellType].drKeys.Length - 1);
        int keyCount = drDictionary[spellType].drKeys[drTier];
        ...
        int invisibleKeys = drDictionary[spellType].drInvisibleKeys[drTier];
    ...
    public void UpdateDRTier(KeyCode spellType, string spellName)
    {
        ...
        if (!drTiers.ContainsKey(spellType))
        {
            return;
        }
        ...
        if (drTiers[spellType] + castMultiplier < drDictionary[spellType].drTimer.Length) {

            drTiers[spellType] += castMultiplier;

        }

        SetCooldown(spellType);
    }
    ...
    public void SetCooldown(KeyCode spellType)
    {
